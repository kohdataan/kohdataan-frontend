...
export default class PostType extends React.PureComponent {
  static propTypes = {
    ...
    componentDidMount() {
      ...
      const duration =
        player.duration > 0 ? player.duration : post.props.duration / 1000

      player.addEventListener('timeupdate', ev => {
        const secs = Math.round(ev.target.currentTime)
        const progress = Math.round(
          (ev.target.currentTime / duration) * 100
        )
        this.setState({
    ...
    this.setState({
        player,
        duration:
            pad2nozero(Math.round(duration) / 60) +
            ':' +
            pad2(Math.round(duration) % 60),
    })
    }
    ...
    render() {
        ...
        return (
          <div>
            <div className="voice-player" style={playerStyle}>
            ...
            <audio id={'voice_' + post.id} preload="none">
                <source
